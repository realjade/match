...
def get_thumbnail(vpath, output, timestr='00:00:03'):
    p = sp.call(
            [FFMPEG_BIN, '-ss', timestr, '-i', vpath, '-an', '-r', '1',
             '-vframes', '1', '-y', output],
            stdin=sp.PIPE,
            stdout=sp.PIPE,
            stderr=sp.PIPE,
...
def convert_to(vpath, output, *args, **kwargs):
    '''
    ffmpeg -i test.mp4 -vcodec libx264 -b 250k -bt 50k -acodec libfaac -ab 56k -ac 2 -s 480x320 out.mp4
    '''
    print 'convert ', vpath, output
    commands = [FFMPEG_BIN,
             '-i', vpath]
    inputs = kwargs.get('inputs', [])
    commands.extend(inputs)
    commands.extend(['-vcodec', 'libx264',
             '-b', '300k',
             '-bt', '50k',
             '-r',  '15',
             '-profile', 'baseline',
    ...
    commands.extend(['-acodec', 'libfaac', '-ab', '56k', '-ac', '2', '-y', output])
    print  ' '.join(commands)
    p = sp.Popen(
            commands,
            stdin=sp.PIPE,
    ...
    while s:
        ...
        while index >= 0:
            if firstreturn:
                index2 = output.rfind('\n')
                print 'first frame...', output[index2+1:index]
                output = output[index+1:]
                firstreturn = False
            else:
                print 'read  frame...', output[:index]
                output = output[index+1:]
            index = output.find('\r')
        s = p.stderr.read(80)
    del p
...
if __name__ == '__main__':
    ...
    #for vformat in get_formats():
    #    print vformat
    #print 'pix_fmts:', get_pixel_formats()
    #print 'info of video:', get_info('test.mp4')
    #print get_thumbnail("test.mp4", '2.jpg')
    convert_to('test.mp4', 'out.mp4')
'''
