...
def user_img_save(filestream,filename):
    """
        个人文件存储
    """
    app = current_app
    user_id = g.user.user_id
    cardfolder = 'userfile/%s/%s' % (user_id,filename,)
    savepath = os.path.join(app.static_folder, cardfolder)
    ...
    try:
        ...
        pass
    filestream.save(savepath)
    fl = Image.open(savepath)
    width, height = fl.size
    return os.path.join(app.static_url_path, cardfolder), width, height


def teacher_card_save(filestream,filename):
    """
        用来存储教师注册提交的身份证和教师资格证的照片
        身份证idcard.* 资格证teacher.*
    """
    app = current_app
    user_id = g.user.user_id
    cardfolder = 'teachercard/%s/%s' % (user_id,filename,)
    savepath = os.path.join(app.static_folder, cardfolder)
    savedir = os.path.dirname(savepath)
    try:
        if not os.path.exists(savedir):
        ...
        pass
    filestream.save(savepath)
    return os.path.join(app.static_url_path, cardfolder)
